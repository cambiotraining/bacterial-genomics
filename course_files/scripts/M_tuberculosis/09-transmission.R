...
snp_matrix_df_decon <- data.frame( t(combn(names(snp_matrix_df),2)), 
...
snp_histogram <- ggplot(data = snp_matrix_df_decon, 
                               aes(x = dist)) + 
...
network_filtered <- tbl_graph(nodes = metadata_nodes_networked_filtered, edges = edges_filtered, directed = TRUE)
